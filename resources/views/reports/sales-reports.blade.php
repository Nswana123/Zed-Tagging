  ...
  <div class="modal-dialog">
    ...
    <div class="col">
        ...
        <div class="col">
            ...
            <select class="form-control" id="ticketedBy" name="product_id">
                <option value="" {{ request('product') == '' ? 'selected' : '' }}>All</option>
        ...
        @else
                                    ...
                                    @php
    $totalAmountCashedIn = 0; // Initialize the total variable
@endphp
                                    <thead>
    <tr>
      <th scope="col">#</th>
      ...
      <th scope="col">Amount Per Unit(ZMW)</th>
      <th scope="col">Amount cashed(ZMW)</th>
      <th scope="col">Sell By</th>
      <th scope="col">Store Name</th>
      <th scope="col">ZedLife App Upsell</th>
      <th scope="col">VoLTE Upsell</th>
  ...
  @foreach($sales as $sale)
            @php
                $amountCashedIn = $sale->quantity * $sale->amount; // Calculate amount cashed in
                $totalAmountCashedIn += $amountCashedIn; // Add to total
            @endphp
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $sale->created_at }}</td>
                                        <td>{{ $sale->msisdn }}</td>
                                        <td>{{ $sale->title }} {{ $sale->fname }} {{ $sale->lname }}</td>
                                        <td>{{ $sale->product->product }} </td>
                                        <td>{{ $sale->quantity }} </td>
                                        <td>{{ $sale->payment_type }} </td>
                                        <td>{{ $sale->amount }} </td>
                                        <td>{{ $sale->amount_cashed_in}} </td>
                                        <td>{{ $sale->user->fname }} {{ $sale->user->lname }}</td>
                                        <td>{{ $sale->user->work_location }}</td>
                                        <td>{{ $sale->zedlife_upsell}}</td>
                                        <td>{{ $sale->volte_upsell }}</td>
                                        <td>{{ $sale->notes }}</td>
                            ...
                            <tfoot>
        <tr>
            <td colspan="8"><strong>Total Amount Cashed In (ZMW):</strong></td>
            <td><strong>{{ $totalAmountCashedIn }}</strong></td> <!-- Display total -->
            <td></td>
        </tr>
    </tfoot>
                                    </table>
