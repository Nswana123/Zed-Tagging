...
@if(!$user_group || $user_group->group_name !== 'Noc field Engineer')
        <div class="row">
            <div class="col">
                <div class="card bg-body shadow-sm">
                    <div class="card-header p-3 text-white" style="background:#0A2558;">
                        <div class="row">
                            <div class="col">
                        All Assigned Tickets<span class="badge badge-danger">{{$allAssignedTickets}}</span>
                </div>
  ...
  @php
    ...
    ($latestResolution && $latestResolution->closed == 'updated' ? 'table-success' : '') 
                                        ...
                                        <td>
                <a href="{{ route('noc_tickets.showAssignedTickets', $ticket->id) }}" class="custom-link" title="Ticket Id">{{ $ticket->case_id }}</a>
            </td>
...
<div class="row mt-3">
            <div class="col">
                <div class="card bg-body shadow-sm">
                    <div class="card-header p-3 text-white" style="background:#0A2558;">
                        <div class="row">
                            <div class="col">
                        My Assigned Tickets<span class="badge badge-danger">{{$MyassignedTickets}}</span>
                </div>
                <div class="col float-left">
                <div class="input-group ">
    <input id="search-focus" type="search" id="form1" class="form-control" placeholder="Search Case Id/Number" />
  ...
  @php
        // Get the latest ticket resolution for the current ticket, if available
        $latestResolution = $ticket->noc_resolutions ? $ticket->noc_resolutions->last() : null;
    @endphp
    
    <tr class="{{ 
        ($latestResolution && $latestResolution->closed == 'update' ? 'table-primary' : '') . ' ' . 
    ($latestResolution && $latestResolution->closed == 'updated' ? 'table-success' : '') 
}}">
                                        <td>{{ $loop->iteration }}</td>
