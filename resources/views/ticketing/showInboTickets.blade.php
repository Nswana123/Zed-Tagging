...
<div class="row">
    ...
    @endif</label>  
            ...
            <div class="card-body">
                               <form action="{{ route('ticketing.updateInbotickets', $ticket->id) }}" method="POST"  enctype="multipart/form-data">
    @csrf
    @method('PUT') 
                                <div class="col-6-ml">
...
<fieldset class="border p-4 mb-4">
    ...
    <div class="col-lg-6 mb-3">
        ...
        <input list="devices" name="device_name" id="deviceInput" class="form-control" placeholder="Select Customer Device" onchange="checkNewDevice()"
        value="{{ optional($ticket->device)->brand ?? 'N/A' }} {{ optional($ticket->device)->model ?? '' }}">
        <input type="hidden" name="device_id" id="device_id" value="{{ optional($ticket->device)->brand ?? 'N/A' }} {{ optional($ticket->device)->model ?? '' }}"
" > <!-- Hidden field to store device_id -->
    ...
    <div class="col-lg-6 mb-3">
        ...
        <input list="locations" name="location_name" id="locationInput" class="form-control" placeholder="Select Customer Location" onchange="checkNewLocation()"
        value="{{ optional($ticket->customerLocation)->province ?? 'N/A'}}, {{ optional($ticket->customerLocation)->town ?? '' }}, {{ optional($ticket->customerLocation)->landmark ?? ''}}">
        <input type="hidden" name="location_id" id="location_id" value="{{ optional($ticket->customerLocation)->province ?? 'N/A'}}, {{ optional($ticket->customerLocation)->town ?? '' }}, {{ optional($ticket->customerLocation)->landmark ?? ''}}"> <!-- Hidden field to store location_id -->
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="root_cause" class="form-control bg-body shadow-sm" Required >
...
<script> 
    document.querySelector('[data-bs-toggle="collapse"]').addEventListener('click', function() {
        ...
        toggleText.textContent = toggleText.textContent === 'Expand' ? 'Collapse' : 'Expand';
    });
 function previewMultipleImages(event) {
    ...
    if (locationInput === 'new_location') {
        ...
        // Only clear location ID if it is not set
        if (!locationIdField.value || locationIdField.value === 'N/A') {
            locationIdField.value = ''; 
        }
    } else {
        newLocationFields.style.display = 'none';

        // Set selected location's ID
...
function checkNewDevice() {
    ...
    if (deviceInput === 'new_device') {
        ...
        // Only clear device ID if it is not set
        if (!deviceIdField.value || deviceIdField.value === 'N/A') {
            deviceIdField.value = '';
        }
    } else {
        newDeviceFields.style.display = 'none';

        // Set selected device's ID
        for (let option of options) {
