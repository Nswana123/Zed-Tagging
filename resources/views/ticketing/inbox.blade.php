...
<div class="row">
    <div class="col">
        <div class="card bg-body shadow-sm">
            ...
            <div class="row">
                <div class="col">
               Inbox  <span class="badge badge-danger">{{$openinboxTickets}}</span>
                </div>
  ...
  <tr class="
        {{ $ticket->opened == 'opened' ? 'table-success' : '' }} 
          {{ $ticket->opened == 'no' ? 'table-danger' : '' }} 
    ">
                                        <td>{{ $loop->iteration }}</td>
                                    
                                        <td>
                <a href="{{ route('ticketing.showInboxTickets', $ticket->tickets->id) }}" class="custom-link" title="Ticket Id">{{ $ticket->tickets->case_id }}</a>
            </td>
                                        <td>{{ $ticket->tickets->created_at }}</td>
                                        <td>{{ $ticket->tickets->msisdn }}</td>
                                        <td>{{ $ticket->tickets->title }} {{ $ticket->tickets->fname }} {{ $ticket->tickets->lname }}</td>
                                        <td>{{ $ticket->tickets->ticket_category->ticket_sla->ttr_in_hour }} hrs</td>
                                        <td>
                                        ...
                                        <td>
                                            @if($ticket->tickets->ticket_category && $ticket->tickets->ticket_category->ticket_sla)
                                                @if($ticket->tickets->ticket_category->ticket_sla->priority == 'severe')
                                                    <span class="badge badge-danger">Severe</span>
                                                @elseif($ticket->tickets->ticket_category->ticket_sla->priority == 'high')
                                                    <span class="badge badge-warning">High</span>
                                                @elseif($ticket->tickets->ticket_category->ticket_sla->priority == 'medium')
                                        ...
                                        <td>{{ $ticket->tickets->ticket_category->category_name ?? 'N/A' }}</td>
                                        <td>{{ Str::limit($ticket->tickets->ticket_category->category_detail ?? 'N/A', 20, ' ...') }}</td>
                                        <td>{{ Str::limit($ticket->tickets->issue_description, 20, ' ...') }}</td>
                                        <td>{{ $ticket->tickets->ticket_status }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                                    </table>
